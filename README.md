...
APP_PASSWORD, UPLOAD_PASSWORD, and DOWNLOAD_PASSWORD are normalized for accidental leading/trailing spaces.
...
The upload and download access endpoints prefer UPLOAD_PASSWORD and DOWNLOAD_PASSWORD. For convenience and to prevent accidental password-variable mismatch, APP_PASSWORD is also accepted for upload/download when it is configured. This does not expose any secret to the browser; passwords remain Render environment variables.
